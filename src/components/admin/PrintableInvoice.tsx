...
export function PrintableInvoice({ invoice }: PrintableInvoiceProps) {
    ...
    useEffect(() => {
        setToday(new Date().toLocaleDateString(undefined, { dateStyle: 'long' }));
    }, []);

    const amountValue = Number(invoice.amount) || 0;
    const isGSTIncluded = !!invoice.isGSTIncluded;
    const parkingAmountValue = Number(invoice.carParkingAmount) || 0;
    
    // Core consistency logic: Trust stored values, otherwise derive precisely
    ...
    const workspaceAmountOnly = subtotalValue - parkingAmountValue;
    ...
    const formattedSubtotal = subtotalValue.toLocaleString('en-IN', {
        style: 'currency',
        currency: 'INR',
        maximumFractionDigits: 0
    });
    const formattedGst = gstValue.toLocaleString('en-IN', {
        style: 'currency',
        currency: 'INR',
        maximumFractionDigits: 0
    });
    ...
    if (typeof window !== 'undefined' && !today) {
        // Prevent initial blank or mismatch during the very first client render if needed,
        // but often just letting useEffect handle it is safer for hydration.
    }
    ...
    return (
        ...
        >
            {/* Header Content */}
            <div className="flex justify-between items-start border-b-[2px] border-primary pb-4 mb-8">
                <div className="space-y-1">
                    <img src={cohortimage.src} alt="Cohort Logo" className="w-[160px] h-auto" />
                    <div className="space-y-0.5 mt-3">
                        <p className="text-[10px] font-black uppercase text-primary tracking-[0.2em] mb-0.5">Official Provider Center</p>
                        <p className="text-sm font-bold text-slate-900">{locationInfo} Center</p>
                        <p className="text-[10px] text-slate-500 font-medium max-w-[280px] leading-tight">{address}</p>
                        <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">GSTIN: 36ABCDE1234F1Z5</p>
                    </div>
                </div>
                
                <div className="text-right flex flex-col items-end">
                    <div className="bg-primary text-white px-8 py-3 rounded-2xl mb-5 shadow-lg">
                        <h1 className="text-2xl font-black italic tracking-tighter uppercase">Tax Invoice</h1>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8 pb-8 border-b border-slate-50">
                <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                    <h4 className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2">
                    ...
                    <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest mt-4 underline decoration-primary/20">Ref: #{invoice._id?.toString().slice(-12).toUpperCase()}</p>
                </div>

                <div className="bg-primary/[0.02] p-6 rounded-[2rem] border border-primary/5 text-right">
                    <h4 className="text-[9px] font-black uppercase text-primary/70 tracking-widest mb-4 flex items-center justify-end gap-2 text-right">
                    ...
                    <p className="text-[8px] font-bold text-slate-300 mt-4 uppercase">Billing Month: {invoice.billingMonth || "Current Booking"}</p>
                </div>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-slate-900 text-white font-black text-[11px] uppercase tracking-widest">
                            <th className="px-8 py-5 text-left">Services Provision Description</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        <tr>
                            <td className="px-8 py-8">
                                <p className="font-black text-slate-900 text-sm mb-1 uppercase tracking-tight">{invoice.workspaceName} Subscription</p>
                                <p className="text-[10px] text-slate-500 font-medium leading-relaxed italic max-w-sm">
                                    Full cycle infrastructure access for {invoice.workspaceName}. Includes membership and ecosystem connectivity.
                                </p>
                            </td>
                            <td className="px-6 py-8 text-center font-bold text-slate-400 text-sm underline decoration-slate-200">01</td>
                            <td className="px-8 py-8 text-right font-black text-slate-900 text-base">
                                {workspaceAmountOnly.toLocaleString('en-IN', {
                                    style: 'currency',
                                    currency: 'INR',
                                    maximumFractionDigits: 0
                                })}
                        ...
                        {parkingAmountValue > 0 && (
                            <tr>
                                <td className="px-8 py-8">
                                    <p className="font-black text-slate-900 text-sm mb-1 uppercase tracking-tight">Reserved Car Parking</p>
                                    <p className="text-[10px] text-slate-500 font-medium leading-relaxed italic max-w-sm">
                                        {invoice.carParkingSlots} slot(s) x {Number(invoice.carParkingPricePerSlot).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 })} per slot.
                                    </p>
                                </td>
                                <td className="px-6 py-8 text-center font-bold text-slate-400 text-sm underline decoration-slate-200">{invoice.carParkingSlots}</td>
                                <td className="px-8 py-8 text-right font-black text-slate-900 text-base">
                                    {parkingAmountValue.toLocaleString('en-IN', {
                                        style: 'currency',
                                        currency: 'INR',
                                        maximumFractionDigits: 0
                                    })}
            ...
            <div className="flex justify-between items-start mb-auto">
                <div className="w-[300px] p-6 bg-slate-50 rounded-[2rem] border border-slate-100 italic">
                    <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-3">Terms & Reminders</p>
                ...
                <div className="w-[340px] space-y-4">
                    <div className="flex justify-between px-4 text-sm font-bold text-slate-500 tracking-tight">
                        <span>Net Subtotal</span>
                        <span>{formattedSubtotal}</span>
                    </div>
                    <div className="flex justify-between px-4 text-sm font-bold text-slate-500 tracking-tight">
                        <span>{isGSTIncluded ? 'GST (18%)' : 'GST (Exempt)'}</span>
                        <span>{formattedGst}</span>
                    </div>
                    <div className="pt-4">
                        <div className="bg-slate-900 text-white rounded-[2.5rem] p-7 flex justify-between items-center shadow-2xl">
                            <div className="flex flex-col">
                                <p className="text-[9px] font-black uppercase tracking-[0.3em] opacity-60 mb-0.5">Final Amount Due</p>
                                <p className="text-[10px] font-bold opacity-80 italic">Amount in INR</p>
                            </div>
                            <p className="text-4xl font-black tracking-tighter italic leading-none">{formattedAmount}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* Official Footing */}
            <div className="flex justify-between items-end border-t border-slate-50 pt-6 mt-8 pb-1">
